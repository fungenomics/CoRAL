...
tools = c('Dummy_Model',paste0('Consensus_',min_agree), strsplit(params$tools, split = ' ')[[1]])
...
for(n in 1:fold){
   ...
   true_train = data.table::fread(paste0(params$pred_path, '/fold', n, '/train_labels.csv'), header = T) %>%
           column_to_rownames('V1') %>% 
     as.data.frame()

   true_train[,'label'] <- apply_ontology(df_ontology = ontology,
                                           pred = true_train[,'label',drop=T],
                                           from = 'label',
                                           to = ontology_columns)
   ...
   for(t in tools){
     ...
     if(t == 'Dummy_Model'){
      ...
      pred.dummy <- sample(x = unique(true_train$label),
                          size = nrow(true),
                          replace = T,
                          prob = as.numeric((table(true_train$label) / nrow(true_train))[unique(true_train$label)])
      )
   
      tmp = data.frame(class = true$label,
                       prediction = pred.dummy
                      )
